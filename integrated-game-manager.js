/**
 * 拡張型ゲームマネージャークラス
 * 元のGameManagerが読み込まれるのを確認してから初期化
 */
(function() {
    // 元のGameManagerが定義されるのを待つ
    function checkGameManagerLoaded() {
      if (typeof GameManager === 'undefined') {
        console.log('Waiting for GameManager to load...');
        setTimeout(checkGameManagerLoaded, 100);
        return;
      }
      
      console.log('GameManager found, initializing enhanced version');
      ...
      class EnhancedGameManager extends GameManager {
        ...
        constructor() {
          ...
          // 操作の連続実行を防ぐフラグ
          this.enhancedOperationInProgress = false;
        ...
        /**
         * 連続操作を防止するデバウンスチェック
         * @returns {boolean} - 操作が許可されるかどうか
         */
        canPerformOperation() {
          if (this.enhancedOperationInProgress) return false;
          this.enhancedOperationInProgress = true;
          setTimeout(() => this.enhancedOperationInProgress = false, 300);
          return true;
        }
        ...
        enhancedClickLyric(element) {
          ...
          if (this.webglRenderer && element) {
            try {
              ...
              console.error('Error in WebGL effects:', e);
            }
          }
        }
