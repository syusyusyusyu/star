  ...
  async loadCaptions() {
    ...
    try {
      ...
      if (response.ok && data.captions && data.captions.length > 0) {
        ...
        const loadingElement = document.getElementById('loading');
        if (loadingElement) {
          let message = '字幕読み込み完了！';
          let color = '#00ff00'; // 緑色で正常
          
          if (data.demo) {
            message = data.message || 'デモ字幕で準備完了！';
            color = '#ffaa00'; // オレンジ色で注意喚起
          } else if (data.method) {
            if (data.isAutoGenerated) {
              message = `自動生成字幕で準備完了！ (${data.language})`;
              color = '#00aaff'; // 青色で自動生成を示す
            } else {
              message = `手動字幕で準備完了！ (${data.language})`;
              color = '#00ff00'; // 緑色で手動字幕を示す
            }
            console.log(`Caption method: ${data.method}`);
          }
          
          loadingElement.textContent = message;
          loadingElement.style.color = color;
        }
