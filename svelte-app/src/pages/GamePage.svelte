<script lang="ts">
  ...
  onMount(async () => {
    ...
    // body クラスを旧HTMLと一致させる
    document.body.classList.add('browser-bar-adjust');
    ...
    // スクリプトを順序通りに読み込み
    const load = (src: string, attrs: Record<string,string> = {}) => new Promise<void>((resolve, reject) => {
      if (document.querySelector(`script[src=\"${src}\"]`)) return resolve();
      const s = document.createElement('script'); s.src = src; s.onload = () => resolve(); s.onerror = (e) => reject(e);
      Object.entries(attrs).forEach(([k,v]) => s.setAttribute(k, v));
      document.body.appendChild(s);
    });

    try {
      await load('https://cdn.tailwindcss.com');
      await load('https://unpkg.com/axios/dist/axios.min.js');
      await load('https://unpkg.com/textalive-app-api/dist/index.js');
      await load('https://cdn.jsdelivr.net/npm/three@0.160.0/build/three.min.js');
      await load('https://cdn.jsdelivr.net/npm/@mediapipe/hands/hands.min.js', { crossorigin: 'anonymous' });
      await load('https://cdn.jsdelivr.net/npm/@mediapipe/camera_utils/camera_utils.js', { crossorigin: 'anonymous' });
      await load('https://cdn.jsdelivr.net/npm/@mediapipe/pose/pose.min.js', { crossorigin: 'anonymous' });
      await load('https://cdn.jsdelivr.net/npm/@mediapipe/drawing_utils/drawing_utils.js', { crossorigin: 'anonymous' });
      await load('https://cdn.jsdelivr.net/npm/@mediapipe/selfie_segmentation/selfie_segmentation.js', { crossorigin: 'anonymous' });
      await load(`${base}game-loader.js`);
      await load(`${base}script.js`);
    } catch (e) {
      console.error('Game scripts load failed', e);
    }
  });
</script>

<!-- 旧 game.html と同一の DOM 構造 -->
