...
> TextAlive App API の歌詞同期、MediaPipe の動作認識AI、Three.js の3D演出のWebリズムゲーム。
...
```
┌─────────────────────────────────────────────────────────────────────────────────────┐
│                              クロステ ～Cross Stage～ 全体アーキテクチャ              │
└─────────────────────────────────────────────────────────────────────────────────────┘

 ┌──── 外部サービス ────────────────────────────────────────────────────────────────┐
 │                                                                                  │
 │  ┌─────────────────┐   ┌──────────────────────┐   ┌──────────────────────────┐  │
 │  │  TextAlive API   │   │  MediaPipe (Google)   │   │  Cloudflare Turnstile    │  │
 │  │  (産総研 AIST)   │   │  Pose / FaceMesh /    │   │  Bot対策 CAPTCHA代替     │  │
 │  │  歌詞同期        │   │  SelfieSegmentation   │   │                          │  │
 │  └────────┬────────┘   └──────────┬───────────┘   └────────────┬─────────────┘  │
 │           │ フレーズの              │ ランドマーク               │ トークン        │
 │           │ タイミング情報          │ 座標データ                 │ 検証            │
 └───────────┼────────────────────────┼────────────────────────────┼────────────────┘
...
│                                                                                      │
│  ┌─ ページ層 ──────────────────────────────────────────────────────────────────┐     │
│  │  IndexPage.tsx (タイトル/モード選択)    GamePage.tsx (ゲーム本体)             │     │
│  │  RankingModal.tsx (ランキング表示)      ModeTabs.tsx / Slot.tsx              │     │
│  └──────────────────────────────────┬──────────────────────────────────────────┘     │
│                                     │                                                │
│  ┌─ ゲームコア層 ───────────────────┴──────────────────────────────────────────┐     │
│  │                          GameManager.ts (司令塔)                             │     │
│  │  ┌──────────────┬──────────────┬──────────────┬──────────────────────┐      │     │
│  │  │ InputManager │LyricsRenderer│EffectsManager│ LiveStageVisuals    │      │     │
│  │  │ 入力統合     │ 歌詞バブル   │ パーティクル │ Three.js 3D演出     │      │     │
│  │  ├──────────────┼──────────────┼──────────────┼──────────────────────┤      │     │
│  │  │ UIManager    │ResultsManager│BodyDetection │ FaceDetection       │      │     │
│  │  │ HUD更新      │ リザルト表示 │ Pose認識     │ FaceMesh認識        │      │     │
│  │  ├──────────────┼──────────────┼──────────────┴──────────────────────┤      │     │
│  │  │ GameLoop     │ BubblePool   │ TimerManager / ViewportManager     │      │     │
│  │  │ 60fps更新    │ DOM再利用    │ タイマー管理 / 表示領域調整        │      │     │
│  │  └──────────────┴──────────────┴────────────────────────────────────┘      │     │
│  └────────────────────────────────────────────────────────────────────────────┘     │
│                                     │                                                │
│  ┌─ サービス層 ─────────────────────┴──────────────────────────────────────────┐     │
│  │  tokenService.ts (トークン取得)         scoreService.ts (スコア送信)         │     │
│  └──────────────────────────────────┬──────────────────────────────────────────┘     │
└─────────────────────────────────────┼────────────────────────────────────────────────┘
                                      │ HTTP通信
    ┌─────────────────────────────────┼──────────────────────────────────┐
    │            API エンドポイント    │                                  │
    │  ┌──────────────────────────────┴──────────────────────────────┐  │
    │  │  GET  /api/token    → HMAC署名トークン発行                  │  │
    │  │  POST /api/score    → スコア登録（多層検証あり）            │  │
    │  │  GET  /api/ranking  → ランキング取得（キャッシュ30秒）      │  │
    │  │  GET  /api/config   → Turnstile Site Key 取得               │  │
...
│                                                                                      │
│  ┌─ ミドルウェア層 ──────────────────────────────────────────────────────────┐       │
│  │  requestId.ts (トレーシング)  session.ts (セッション管理)  admin.ts (管理認証)│    │
│  └────────────────────────────────────┬──────────────────────────────────────┘       │
│                                       │                                              │
│  ┌─ ルート層 ─────────────────────────┴──────────────────────────────────────┐       │
│  │  score.ts (スコア/ランキング)           admin.ts (管理API)                 │       │
│  └────────────────────────────────────┬──────────────────────────────────────┘       │
│                                       │                                              │
│  ┌─ セキュリティ (多層防御) ──────────┴──────────────────────────────────────┐       │
│  │  1. Origin検証 → 2. Rate Limiter → 3. HMAC署名検証 → 4. Nonce検証       │       │
│  │  → 5. Turnstile検証 → 6. スコア上限チェック (is_suspicious フラグ)       │       │
│  └────────────────────────────────────┬──────────────────────────────────────┘       │
│                                       │                                              │
│  ┌─ サービス層 ───────────────────────┴──────────────────────────────────────┐       │
│  │  scoreService.ts       rankingCache.ts (30秒TTL)       rateLimiter.ts    │       │
│  │  (検証/保存/取得)      (メモリキャッシュ)              (IP別制限/Nonce)   │       │
│  └────────────────────────────────────┬──────────────────────────────────────┘       │
└───────────────────────────────────────┼──────────────────────────────────────────────┘
                                        │ PostgreSQL
                                        ▼
...
│                                                                                      │
│  ┌─ scores テーブル ─────────────────────────────────────────────────────────┐       │
│  │  id | session_id | song_id | mode | speed | score | max_combo | rank     │       │
│  │  accuracy | is_suspicious | player_name | created_at                      │       │
│  └───────────────────────────────────────────────────────────────────────────┘       │
└─────────────────────────────────────────────────────────────────────────────────────┘
 ...
 │  【ゲームプレイ〜スコア登録の流れ】                                              │
 │                                                                                  │
 │  1. IndexPage でモード(cursor/body/mobile/face) & 速度(8/10/12秒) を選択        │
 │  2. GamePage → GameManager 初期化 → TextAlive Player 起動 & カメラ起動          │
 │  3. GameLoop(60fps) で毎フレーム:                                                │
 │       TextAlive → 現在時刻取得 → 歌詞バブル生成/配置                            │
 │       InputManager/MediaPipe → プレイヤー入力検知 → ホールド判定 → スコア加算   │
 │  4. 楽曲終了 → ResultsManager がリザルト表示                                    │
 │  5. GET /api/token → HMAC署名トークン取得                                       │
 │  6. POST /api/score → 多層検証 → Supabase に保存                               │
 │  7. GET /api/ranking → キャッシュ付きランキング取得 → 順位表示                  │
 │                                                                                  │
 └──────────────────────────────────────────────────────────────────────────────────┘
```
...
### なぜ「設計」が大事なのか ― 実体験から学んだこと

クロステは当初、通常のJavaScriptで1ファイルにすべてのロジックを書いていました。
しかし機能が増えるにつれ、コードが肥大化し修正や機能追加が困難になりました。

```
【開発初期】Vanilla JS / 1ファイル構成
script.js (5000行超)
  - 入力処理 / 描画処理 / スコア計算 / API通信 / エフェクト / カメラ認識 ...
  → どこに何があるかわからない！1箇所の変更が全体に波及して怖い！
```

この問題を解決するために、オブジェクト指向開発における **SOLID原則** の **S（単一責任の原則: Single Responsibility Principle）** を採用。
「1つのクラスは、1つのことだけに責任を持つべき」という考え方に従い、TypeScript への移行とともにクラス分割を行いました。

```
【現在】TypeScript / Manager パターンで責務分離
GameManager.ts    - 全体の進行を管理
InputManager.ts   - 入力だけ担当
LyricsRenderer.ts - 歌詞の描画だけ担当
ScoreService.ts   - API通信だけ担当
→ 見通しが良い！修正も安心！各ファイルが独立して変更可能！
```

この設計判断により、後から追加したモバイルモード・フェイスモード・速度コースなどの機能も、既存コードへの影響を最小限に抑えて実装できました。
...
テーブルは **`scores` の1つだけ**。ユーザー管理テーブルやセッションテーブルは持たず、匿名プレイ＆スコア記録に必要な情報をこの1テーブルに集約しています。複雑なリレーションがないため、クエリもシンプルで高速です。
