...
## 1. システム概要
Cross Stage は、TextAlive による歌詞同期と MediaPipe による身体/顔検知を組み合わせた Web リズムゲームです。プレイモードは cursor/body/mobile/hand/face を提供します。フロントエンドは React/Vite の SPA、バックエンドは Cloudflare Workers + Hono、データストアは Supabase (PostgreSQL) を利用します。

```mermaid
flowchart LR
  Player[プレイヤー] -->|操作| FE["Frontend (React/Vite)"]
  FE -->|歌詞同期| TextAlive["TextAlive App API"]
  FE -->|姿勢/手/顔/人物抽出| MediaPipe["MediaPipe Pose/Hands/FaceMesh/SelfieSegmentation"]
...
flowchart TB
  ...
  subgraph Camera[カメラ認識]
    C_In[Input: Webカメラ映像]
    C_Proc[Process: Pose/Hands/FaceMesh/Segmentation, 品質/負荷制御]
...
flowchart LR
  ...
  Client -->|Pose/Hands/FaceMesh/Segmentation| MediaPipe
```
...
| mode | text | NOT NULL, default 'cursor' | cursor/body/mobile/hand/face |
...
| 入力/カメラ | マウス/タッチ/カメラ入力、Pose/Hands/Face 判定 | src/game/GameManager.ts (InputManager, BodyDetectionManager, FaceDetectionManager) |
...
| CAM-02 | フェイスモード | 機能 | 口を開閉してバブルに近づける | 開口時にホールド判定、閉口で解除 |
